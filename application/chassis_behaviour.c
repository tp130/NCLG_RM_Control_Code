...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
    {
        ...
        //CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW,CHASSIS_NO_FOLLOW_YAW,CHASSIS_OPEN
			if(chassis_move_mode->chassis_RC->key.v & CHASSIS_SPIN_FAST)
			{	
				chassis_move_mode->chassis_sub_mode = CHASSIS_SUB_UPUP;//全速
				chassis_behaviour_mode = CHASSIS_SPIN_MODE;
			}
			else if(chassis_move_mode->chassis_RC->key.v & CHASSIS_SPIN_LOW)
			{
				chassis_move_mode->chassis_sub_mode = CHASSIS_SUB_OBLI;//半速
				chassis_behaviour_mode = CHASSIS_SPIN_MODE;
			}
			else
			{
    ...
    else if (chassis_behaviour_mode == CHASSIS_OPEN)
    {
		...
		else if(chassis_behaviour_mode== CHASSIS_SPIN_MODE)
		{
			chassis_move_mode->chassis_mode = CHASSIS_SPINING;
		}
}
...
static void chassis_spin_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
		...
		//if(chassis_move_rc_to_vector->vx_set)
    *wz_set = 8*(8-fabs(chassis_move_rc_to_vector->vx_set)-fabs(chassis_move_rc_to_vector->vy_set))/8;  //变速模式
}
