...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if(gimbal_behaviour == GIMBAL_SPINING)
		{
				gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_SPIN;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_SPIN;
		}
}
...
static void gimbal_absolute_spin_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    if (yaw == NULL || pitch == NULL || gimbal_control_set == NULL)
    {
		...
		*yaw += gimbal_control_set->gimbal_rc_ctrl->rc.ch[YAW_CHANNEL]* YAW_RC_SEN+gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN;
		//yaw_channel * Yaw_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * Yaw_Mouse_Sen
}
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    {
			if(gimbal_mode_set->gimbal_rc_ctrl->key.v &(CHASSIS_SPIN_FAST | CHASSIS_SPIN_LOW))
			{
				gimbal_behaviour =  GIMBAL_SPINING;
			}
			else
			{
