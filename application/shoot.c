...
static void shoot_set_mode(void)
{
    ...
    //注释掉处于中档， 
		//随时使用键盘开启摩擦轮  Q键开启摩擦轮
    if ((shoot_control.shoot_rc->key.v & SHOOT_ON_KEYBOARD) && shoot_control.shoot_mode == SHOOT_STOP)//switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) &&
    { 
				shoot_laser_on(); //激光开启
        shoot_control.shoot_mode = SHOOT_READY_FRIC;
    }
    //注释掉处于中档， 处于中档， 
		//随时使用键盘关闭摩擦轮 E键关闭摩擦轮
    else if ( (shoot_control.shoot_rc->key.v & SHOOT_OFF_KEYBOARD) && shoot_control.shoot_mode != SHOOT_STOP)//switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) &&
    {
    ...
    if(shoot_control.shoot_mode > SHOOT_READY_FRIC)
    {
        ...
        else if((shoot_control.press_l_time == PRESS_SHORT_TIME))
        {
            shoot_control.shoot_mode =SHOOT_BULLET;
        }
				else
...
static void shoot_feedback_update(void)
{
    ...
    {
        if (shoot_control.press_l_time < PRESS_SHORT_TIME)
        {
