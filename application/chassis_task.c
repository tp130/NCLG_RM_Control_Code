...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_RAW)
    {
		...
		else if (chassis_move_control->chassis_mode == CHASSIS_SPINING)
    {
        fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
        //旋转控制底盘速度方向，保证前进方向是云台方向，有利于运动平稳
        sin_yaw = arm_sin_f32(-chassis_move_control->chassis_yaw_motor->relative_angle);
...
{
	if(chassis_move.chassis_sub_mode == CHASSIS_SUB_SUPPLY||chassis_move.chassis_sub_mode == CHASSIS_SUB_OBLI)
 {
		wheel_speed[0] = 0.5f*(-vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
    wheel_speed[1] = 0.5f*(vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
    wheel_speed[2] = 0.5f*(vx_set + vy_set + (-CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
    wheel_speed[3] = 0.5f*(-vx_set + vy_set + (-CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
 }
	 else if(chassis_move.chassis_sub_mode == CHASSIS_SUB_UPUP)
 {
 ...
 else if(chassis_move.chassis_mode == CHASSIS_SPIN_MODE)
	{
    wheel_speed[0] = 0.5f*(-vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
		wheel_speed[1] = 0.5f*(vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
		wheel_speed[2] = 0.5f*(vx_set + vy_set + (-CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set);
